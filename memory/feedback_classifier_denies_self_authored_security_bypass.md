...
- Note: drift-gate bypass requires TWO files: `skip_commit_drift_gate` (model-writeable, reason) AND `bypass_commit_drift_approved` (user-writeable, approval marker). The split is by design — model authors reasoning, user authorizes.
