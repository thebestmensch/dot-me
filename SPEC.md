...
**Optional derived companion: `voice.compact.md` (v0.4).** A small (~30-line) always-loaded extract of `voice.md` that carries the top-tier voice signal: the identity-invariant rules and the loudest AI-speak tells. It exists to solve the lazy-load gap in §6.1 — a consumer that can't do conditional loading (or an agent that doesn't notice it needs voice) still gets enough signal at session start to keep first-pass prose from defaulting to model house style. It is **derived from `voice.md`, not a fifth core file**: same status as the integrity sidecar (implementation choice), generated by the producer, optional for any consumer. Its load contract is in §6.1; its shape and the prose-not-data rule are in §5.2. Consumers MUST work correctly whether or not it exists.
...
##### 5.2.1. `voice.compact.md` (optional derived companion, v0.4)

`voice.md` is rich (often 10-20 KB) and §6.1 loads it conditionally — only when the agent expects to generate user-facing text. That conditional load fails silently when the agent doesn't notice the task is prose-shaped (a "pick a tagline" request is a copy decision wearing a config-pick costume), skips the cue under context-budget pressure, or runs in a consumer with no skill-discovery semantics to fire an on-demand load. The result: the agent has every fact about the user and zero voice signal, so the first draft defaults to model house style (corporate, em-dash-heavy, "It's worth noting that...").

`voice.compact.md` closes that gap. It is a short extract of `voice.md` — the producer's call on what's top-tier, but in practice the **identity-invariant rules** (the mechanics that hold across every register: no em-dashes, no semicolons, no corporate phrases, connective-driven flow) and the **loudest AI-speak tells to strip on sight**. It is small enough (~2 KB) to load every session at negligible cost, and it ends with a pointer back to the full `voice.md` for any real prose work.

Rules:

- **Derived, not authoritative.** `voice.md` is the source of truth. `voice.compact.md` is a lossy extract; on any conflict, the full file wins. A consumer that has both loaded SHOULD treat the full file as authoritative.
- **Prose, not data.** Same rule as `voice.md` (§5.2): consumers load it as plain prose, never parse it as structured data.
- **Generation is the producer's job.** For v0.4, a hand-maintained extract is fine (the reference `/me` producer keeps it in sync by convention). Automated generation from `voice.md` is deferred; the spec does not mandate a generator.
- **Optional.** A producer that doesn't generate it loses the default-voice-exposure benefit but stays fully conformant. A consumer that doesn't load it stays fully conformant (§6.1).
...
- `voice.compact.md` (if present): load every session, like `identity.yaml`. It is small (~2 KB) and exists precisely to be always-on, so a prose task in an otherwise non-prose session doesn't start from house style. This is the **default voice exposure** for a consumer that can't reliably do the conditional `voice.md` load below.
- `voice.md`: load when the agent generates user-facing text (writing, replying, drafting, generating commit messages). A math-only or pure-code-fix context can skip it. When `voice.compact.md` is always-loaded, the full file is the *deep* load — fired for real prose work (a bio, a blog draft, register-specific output), not every session.
...
**Default voice exposure (v0.4).** Conditional loading has a silent-failure mode: the agent decides voice isn't needed and is wrong, and nothing surfaces the miss — the draft just reads as house style. The cheapest robust answer is to **always-load a compact voice signal** (`voice.compact.md`) so the floor is never "zero voice," and reserve the full `voice.md` for the deep load. A consumer SHOULD ensure *some* voice signal reaches every session that might generate prose; always-loading the compact companion is the recommended way. Consumers that genuinely can do reliable conditional loading (a skill the agent fires on prose triggers) MAY skip the always-load and lazy-load `voice.md` directly — but the always-load floor is the safer default, and the two compose (compact always on, full fired on demand).

**Consumer coverage as of 2026-06-18.** The reference Claude Code plugin in this repo auto-imports `identity.yaml` via the `@~/.me/identity.yaml` line `/me init` writes into `~/.claude/CLAUDE.md` (the `me-integrity.sh` SessionStart hook is a separate concern — it baseline-checks for tamper drift, it does not itself load content), and recognises all four core files in its `/me` management surface (`show`, `edit`, `add`, `init`, `check`, integrity baseline). The v0.4 recommended wiring for the always-loaded voice floor is the same `@`-import mechanism: add `@~/.me/voice.compact.md` to the consumer's startup config. Folding that line into `/me init` and adding `voice.compact.md` to the plugin's `/me` management + integrity baseline is the tracked reference-plugin follow-up. The full `voice.md`, `preferences.yaml`, and `working-style.yaml` are loaded by the consumer's own wiring or by skills that need them. The Codex / Cursor / Gemini installers in `consumers/` currently inline only `identity.yaml` (and optionally `preferences.yaml` / `voice.md`); `working-style.yaml` support in those installers is tracked as a follow-up. Adopters using those installers SHOULD apply the conformance disclosure rule below. A tool that does not yet load `working-style.yaml` will not honor its rules.
...
**Optional third pattern: prompt-trigger injection (the enforcement backstop).** Always-loading `voice.compact.md` (§6.1) sets the floor; a consumer that wants belt-and-suspenders for the *full* voice on prose tasks MAY add a runtime hook that detects prose-generation intent in the user's prompt (keywords like `draft`, `bio`, `blurb`, `rewrite`, `email`, `reply`, `post`) and injects the full `voice.md` (or fires the consumer's voice skill) for that turn. This is a per-consumer convenience, not a spec requirement — and it has a hard boundary:

> A prompt-trigger hook MUST inject dot-me content **into the instruction context** (the same tier as session-start loading). It MUST NOT satisfy the trigger by exposing a new model-callable surface that returns `~/.me/` content as a tool result — that is exactly the exfiltration vector §6.3 prohibits. Injecting text at prompt-submit time is harness-side context assembly, not a retrievable tool; auto-firing a `get_user_voice()`-style tool call is not. The distinction is *who* pulls the content: the harness (allowed) vs. the model deciding to call a fetch tool (forbidden).

A reference Claude Code `UserPromptSubmit` hook implementing this pattern ships in [`examples/voice-enforcement-hook/`](examples/voice-enforcement-hook/).
...
- `.integrity`: a SHA-256 hash baseline of the four content files (`identity.yaml`, `voice.md`, `preferences.yaml`, `working-style.yaml`) plus `voice.compact.md` when the producer generates it, regenerated on every legitimate write. The compact companion belongs in the baseline because §6.1 always-loads it — an always-loaded file is the same tamper surface as `identity.yaml`, so leaving it unbaselined would be a blind spot.
...
- Should the spec mandate (or reference-implement) automated `voice.compact.md` generation from `voice.md`, rather than the v0.4 hand-maintained-by-convention approach? Open risk: a stale compact file silently ships outdated voice signal to every session.
- Should `voice.md` allow optional YAML frontmatter for structured metadata (e.g., `voice_version`) without breaking the prose-not-data rule? (Would also give `voice.compact.md` a derivation-provenance anchor.)
