...
- **[v0.5.0](RELEASE_NOTES_v0.5.0.md)** (2026-06-18): `education[]` optional `identity.yaml` block (JSON Resume education shape; the sibling v0.3 deferred). Reference `/me linkedin-import` producer subcommand maps a LinkedIn export onto the EXISTING schema (`Profile.csv` → location/website/social_profiles, `Positions.csv` → `work[]`/`past_work[]`, `Education.csv` → `education[]`) — idempotent, non-destructive, dry-run-first. Additive: existing `~/.me/` content stays valid; `education[]` and the importer are both optional.
